...
export const FloatingWhatsAppButtons: React.FC<FloatingWhatsAppButtonsProps> = ({
  ...
  useEffect(() => {
    // Enable continuous destination jumping after all icons have finished dropping in
  ...
  return (
    <div 
      ...
      className="fixed bottom-20 right-3 sm:bottom-24 sm:right-6 z-40 flex flex-col gap-3 items-end pointer-events-none"
    >
      {/* 1ST (TOP) WHATSAPP BUTTON: Canada Desk (+1 587 838 9106) - Positioned on top */}
      <motion.div
        className="pointer-events-auto relative flex items-center justify-end group"
        initial={{ y: -900, opacity: 0, scale: 0.6, rotate: -20 }}
        animate={
          hasLanded
            ? {
                y: [0, -14, 2, -7, 0],
        ...
        transition={
          hasLanded
            ? {
                ...
                repeat: Infinity,
                repeatDelay: 4,
                delay: 0.2, // Alternates jumping rhythm
                ease: "easeInOut",
              }
            : {
                type: "spring",
                ...
                stiffness: 140,
                mass: 1.1,
                delay: 0.2, // Drops from top first
              }
        }
      >
        {/* Pulsing Aura Rings at destination */}
        <span className="absolute inset-0 rounded-full bg-rose-500/30 animate-ping pointer-events-none -z-10" />
        <span className="absolute -inset-1 rounded-full bg-gradient-to-r from-emerald-500 to-rose-500 opacity-60 blur-xs animate-pulse pointer-events-none -z-10" />

        {/* Hover Tooltip Card */}
        <div className="absolute right-14 sm:right-16 top-1/2 -translate-y-1/2 bg-[#0D0D0D] border border-rose-500/50 text-slate-100 text-xs py-2 px-3.5 rounded-2xl opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 whitespace-nowrap shadow-[0_10px_30px_rgba(0,0,0,0.85)] flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-rose-400 animate-pulse"></span>
          <div>
            <span className="font-bold text-white block leading-tight">Canada Direct Consular Desk</span>
            <span className="text-[10px] text-rose-300 font-mono">{whatsAppDisplay3}</span>
          </div>
        </div>

        {/* Action Anchor Button */}
        <motion.a
          href={`https://wa.me/${whatsAppNum3}?text=Hello%20ConsulPortal%20Immigration%20Team%2C%20I%20am%20interested%20in%20your%20overseas%20vacancies%20and%20visa%20processing%20services.`}
          target="_blank"
          rel="noopener noreferrer"
          id="whatsapp-floating-ca-btn"
          whileHover={{ scale: 1.18, rotate: [0, -8, 8, 0] }}
          whileTap={{ scale: 0.9 }}
          className="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-gradient-to-br from-emerald-500 via-emerald-600 to-rose-700 text-slate-950 flex items-center justify-center shadow-[0_8px_25px_rgba(225,29,72,0.45)] border-2 border-rose-400/80 cursor-pointer relative"
          title={`Chat with Canada WhatsApp (${whatsAppDisplay3})`}
        >
          {/* WhatsApp Official Vector Path */}
          <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)]">
          ...
          {/* Region Tag Badge */}
          <span className="absolute -top-1 -right-1 bg-[#141414] text-rose-300 border border-rose-400/80 text-[8px] sm:text-[9px] font-black px-1.5 py-0.5 rounded-full font-mono shadow-md">
            CA
          </span>
      ...
      {/* 2ND (MIDDLE) WHATSAPP BUTTON: US / International Helpline */}
      <motion.div
        className="pointer-events-auto relative flex items-center justify-end group"
      ...
      >
        ...
        <motion.a
          ...
          whileHover={{ scale: 1.18, rotate: [0, -8, 8, 0] }}
          whileTap={{ scale: 0.9 }}
          className="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-gradient-to-br from-emerald-400 via-emerald-500 to-emerald-600 text-slate-950 flex items-center justify-center shadow-[0_8px_25px_rgba(16,185,129,0.55)] border-2 border-[#D4AF37]/70 cursor-pointer relative"
      ...
      {/* 3RD (BOTTOM) WHATSAPP BUTTON: UK / European Desk */}
      <motion.div
        className="pointer-events-auto relative flex items-center justify-end group"
        initial={{ y: -900, opacity: 0, scale: 0.6, rotate: 25 }}
        animate={
        ...
        transition={
          hasLanded
            ? {
                ...
                repeat: Infinity,
                repeatDelay: 4,
                delay: 0.7, // Alternates jumping rhythm
                ease: "easeInOut",
              }
            : {
                type: "spring",
                ...
                stiffness: 140,
                mass: 1.1,
                delay: 0.7, // Drops from top third
              }
        }
      >
        {/* Pulsing Aura Rings at destination */}
        ...
        <motion.a
          ...
          whileHover={{ scale: 1.18, rotate: [0, 8, -8, 0] }}
          whileTap={{ scale: 0.9 }}
          className="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-gradient-to-br from-emerald-500 via-teal-600 to-emerald-700 text-slate-950 flex items-center justify-center shadow-[0_8px_25px_rgba(20,184,166,0.55)] border-2 border-blue-400/70 cursor-pointer relative"
